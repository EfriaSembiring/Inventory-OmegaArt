...
<?php 
    if (isset($_POST['bulan'])){
        ...
        echo "<script> $('title').text('DAFTAR BARANG KELUAR BULAN ".strtoupper(bulan($_POST['bulan']))." TAHUN ".$tahun."'); </script>";
    }else {
        echo "<h4>Menampilkan Semua Data</h4>";
        echo "<script> $('title').text('DAFTAR SEMUA BARANG KELUAR'); </script>";
    }
?>
        ...
        <tbody>
            <?php
                ...
                if (isset($_POST['bulan'])){
                    $bulan=$_POST['bulan'];
                    $tahun=$_POST['tahun'];
                    $sql="select * from barang_keluar m inner join detail_barang_keluar d on d.kode_transaksi=m.kode_transaksi where month(m.tanggal)='".$bulan."' and year(m.tanggal)='".$tahun."' group by m.kode_transaksi order by m.tanggal desc";
                }else {
                    $sql="select * from barang_keluar m inner join detail_barang_keluar d on d.kode_transaksi=m.kode_transaksi group by m.kode_transaksi order by m.tanggal desc";
                }
                ...
                $query1=mysqli_query($kon,"select kode_transaksi from detail_barang_keluar where kode_transaksi='".$data['kode_transaksi']."'");
                $jumlah_barang = mysqli_num_rows($query1);

                $query2=mysqli_query($kon,"select sum(qty) as total_qty, sum(harga*qty) as total_harga from detail_barang_keluar where kode_transaksi='".$data['kode_transaksi']."'");
...
$('.tombol_edit_transaksi').on('click',function(){
    ...
    $.ajax({
        url: 'pages/barang-keluar/edit-transaksi.php',
        method: 'post',
        data: {kode_transaksi:kode_transaksi},
        success:function(data){
            $('#tampil_data').html(data);  
                document.getElementById("judul").innerHTML='Edit Transaksi Barang Keluar';
        }
    });
    // Membuka modal
    $('#modal').modal('show');
...
$('.tombol_hapus_transaksi').on('click',function(){
    ...
    if (!agree){
        ...
        var kode_transaksi = $(this).attr("kode_transaksi");
        $.ajax({
            url: 'pages/barang-keluar/ajax/hapus.php',
            method: 'post',
            data: {kode_transaksi:kode_transaksi},
            success:function(data){
                tabel_transaksi();
